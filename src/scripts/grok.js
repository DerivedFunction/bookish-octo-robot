...
async function getTextInput(maxRetries = 10, retryDelay = 3000) {
  let { query, queryEngines, Grok } = await chrome.storage.local.get([
    "query",
    "queryEngines",
    "Grok",
  ]);
  const searchQuery = (queryEngines && Grok ? queryEngines : query)?.trim();
